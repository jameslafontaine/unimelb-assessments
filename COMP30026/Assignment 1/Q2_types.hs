module Q2_types (Exp(..), parse, pprint)
where

import Data.Char

data Exp
  = VAR  Char     -- Variable (Propositional Letter)
  | NOT  Exp      -- Negation
...
pprint TRUE = "true"
pprint (VAR v) = [v]
pprint (IMPL p q) = "(" ++ pprint p ++ "=>" ++ pprint q ++ ")"
pprint (AND p q) = "(" ++ pprint p ++ "&" ++ pprint q ++ ")"
pprint (NOT p) = "(~" ++ pprint p ++ ")"
pprint (OR p q) = "(" ++ pprint p ++ "|" ++ pprint q ++ ")"
pprint (BIIM p q) = "(" ++ pprint p ++ "<=>" ++ pprint q ++ ")"
pprint (XOR p q) = "(" ++ pprint p ++ "<+>" ++ pprint q ++ ")"
